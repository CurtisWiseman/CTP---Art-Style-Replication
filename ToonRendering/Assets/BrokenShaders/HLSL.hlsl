Shader "CG shader for toon shading compatible with Unity"
{
	Properties
	{
	...
	SubShader
	{
		Pass
		{
			...
			varying float3 varyingNormalDirection;
			...
			void main()
			{
				...
				varyingNormalDirection = normalize(float3(
					float4(gl_Normal, 0.0) * modelMatrixInverse));

				SV_Position = gl_ModelViewProjectionMatrix * gl_Vertex;
			}
			...
			void main()
			{
				float3 normalDirection = normalize(varyingNormalDirection);
				...
				SV_Target = float4(fragmentColor, 1.0);
			}
